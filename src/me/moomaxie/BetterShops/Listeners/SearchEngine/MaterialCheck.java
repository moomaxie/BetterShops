...
public class MaterialCheck {
    ...
    public static void searchByMaterial(Inventory inv, Player p, Shop shop, String search, boolean sell) {
        ...
        if (Material.getMaterial(search.toUpperCase()) != null) {
            items.add(Material.valueOf(search.toUpperCase()));
            ...
            for (Material m : Material.values()) {
                ...
                for (int i = 0; i < split.length; i++) {
                    ...
                    if (m.name().contains(split[i].toUpperCase())) {
                        items.add(m);
                    }
                }
            }
            ...
            for (Material m : Material.values()) {
                if (m.name().contains(search.toUpperCase().replaceAll(" ", "_"))) {
                ...
                for (String aSplit : split) {
                    ...
                    if (m.name().contains(aSplit.toUpperCase())) {
                        items.add(m);
                    }
                }
            }
        ...
        openShopWithSearch(inv, p, shop, 1, sell, items);
    }

    private static void openShopWithSearch(Inventory inv, Player p, Shop shop, int page, boolean sell, List<Material> items) {
        ...
        for (ShopItem it : shop.getShopItems(sell)) {
            if (items.contains(it.getItem().getType())) {
